document.addEventListener('DOMContentLoaded', function() {
    const initPedidosVista = () => {
        ...
        // Función global para imprimir pedido
        window.imprimirPedido = function(pedidoId) {
            showAlert({
                type: 'info',
                title: 'Preparando impresión...',
                message: 'Generando documento para impresión',
                duration: 3000
            });
            ...
            setTimeout(() => {
                showAlert({
                    ...
                    message: 'El documento se ha preparado. Se abrirá la ventana de impresión.',
                    duration: 3000
                });
        ...
        // Alertas informativas al cargar la página
        setTimeout(() => {
            const totalPedidos = document.querySelectorAll('.pedido-card').length;
            if (totalPedidos === 0) {
                const esPaginaRecibidos = window.location.pathname.includes('recibidos');
                showAlert({
                    type: 'info',
                    title: esPaginaRecibidos ? 'No hay pedidos recibidos' : 'No hay pedidos enviados',
                    message: esPaginaRecibidos ? 
                        'Los pedidos de tus clientes aparecerán aquí cuando los recibas' :
                        'Los pedidos que realices a proveedores aparecerán aquí',
                    duration: 5000
                });
            } else {
                showAlert({
                    ...
                    message: `Se encontraron ${totalPedidos} pedido(s) en total`,
                    duration: 3000
                });
            }
        }, 1000);
